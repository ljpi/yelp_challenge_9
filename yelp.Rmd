---
title: "Yelpers in The Great Recession: A Case Study"
date: "2/7/2017"
output:
...
This is a case study into Yelp restaurantgoers' consumer behavior during The Great Recession based off the Yelp Challenge 9.
...
Early Yelp data is not very good
Most recent Yelp data does not include entire month

subset relevant data from begining of 2006 to end of 2016

Recession only happened in US, data from worldwide

#Motivation
If restaurant Yelpers' behavior during and aroud The Great Recession period can be modeled,then there can be conclusions about Yelpers' behavior. If recessionary behavior can be understood, then restaurants can react accordingly.

#The Process

After gathering the data, I examined all the potentially relevant information.

Then gathered any additional data that may help

Then test to see if assumptions about the inner workings and dynamics were true

Finally, try to model the consumer's behavior
...
#Users, Stocks, & Reviews

First, I will explore the Yelp stock data to see if it is of any relevance in an attempt to handle any cases of endogeneity.
...
Growth rate of users do not have an effect on the stock value of Yelp - no correlation to stock market. good! This elinates the possibility that effects are due to yelp as a company doing good or bad. Eliminate stock data from our study
...
<!-- rect(2007.9166667, -1, 2009.4166667, 1, col=rgb(red=150/255,green=25/255,blue=25/255,alpha=.3), border=NA) -->
...
#how bout use stars in reg for dolla dolla
```{r}



lm_d1_adj_rev_adj_star = lm(tslm_d1_resid~recession_dummy_dollars_m+ts_rev_count_adj+df_dollars_1_star$avg)
summary(lm_d1_adj_rev_adj_star)

lm_d2_adj_rev_adj_star = lm(tslm_d2_resid~recession_dummy_dollars_m+ts_rev_count_adj+df_dollars_2_star$avg)
summary(lm_d2_adj_rev_adj_star)

lm_d3_adj_rev_adj_star = lm(tslm_d3_resid~recession_dummy_dollars_m+ts_rev_count_adj+df_dollars_3_star$avg)
summary(lm_d3_adj_rev_adj_star)

lm_d4_adj_rev_adj_star = lm(tslm_d4_resid~recession_dummy_dollars_m+ts_rev_count_adj+df_dollars_4_star$avg)
summary(lm_d4_adj_rev_adj_star)

vif(lm_d1_adj_rev_adj_star)
vif(lm_d2_adj_rev_adj_star)
vif(lm_d3_adj_rev_adj_star)
vif(lm_d4_adj_rev_adj_star)

#not really a difference, star ratings dynamics not really linked to review numbers, but both effected by rec in dif ways
...
need to convert number of reviews to be on same scale as rest expenditure growth rates
...
#only the lags
lm_temp = lm(log_rev_quarter_adj~rest_real_exp_diff_log)
...
#build rest exp into the dollar signs models
```{r}
...
combinedFull1 = cbind(ts_d1_quarterly,rec_q,ts_rev_count_adj_q,ts_d1_star_quarterly,rest_exp_q_adj)
...
combinedFull2 = cbind(ts_d2_quarterly,rec_q,ts_rev_count_adj_q,ts_d2_star_quarterly,rest_exp_q_adj)
...
combinedFull3 = cbind(ts_d3_quarterly,rec_q,ts_rev_count_adj_q,ts_d3_star_quarterly,rest_exp_q_adj)
...
combinedFull4 = cbind(ts_d4_quarterly,rec_q,ts_rev_count_adj_q,ts_d4_star_quarterly,rest_exp_q_adj)
