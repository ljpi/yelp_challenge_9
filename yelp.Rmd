...
#usethis
users_by_date = with(users_by_date_all, users_by_date_all[(users_by_date_all$yelping_since >= "2005"), ])
...
yelp_stock = get.hist.quote("YELP",end="2017-02-01",quote="AdjClose",compression="m")
yelp_stock
...
restaurant_reviews = with(restaurant_reviews_all, restaurant_reviews_all[(restaurant_reviews_all$date >= "2005"), ])
...
log_rev_count = ts(log_rev_count,start=c(2005,2),freq=12)

ts_m
log_user_count=diff(log(ts_m[,1]))
log_user_count = na.omit(log_user_count)
log_user_count = ts(log_user_count,start=c(2005,2),freq=12)

plot(log_rev_count,type="l", main="growth rate of user reviews and accounts")
lines(log_user_count[,1],col="red")

diff_logs = log_rev_count-log_user_count
lines(diff_logs,col="blue")
...
vm_rates=VAR(rates_combined,p=12)
plot(vm_rates$y)
summary(vm_rates)
...
plot(gdp_growth)
gdp_growth_subset = with(gdp_growth, gdp_growth[index(gdp_growth) >= "2004-12-01", ])
gdp_growth_subset = ts(gdp_growth_subset,start=c(2005,1),frequency = 4)

#create quarterly user growth rate
...
log_rev_quarter = ts(log_rev_quarter,start=c(2005),freq=4)
...
#plot(decompose_ts_dollar_1$seasonal)
#plot(decompose_ts_dollar_1$trend)
#plot(decompose_ts_dollar_1$random)
...
#plot(decompose_ts_dollar_2$seasonal)
#plot(decompose_ts_dollar_2$trend)
#plot(decompose_ts_dollar_2$random)
...
#plot(decompose_ts_dollar_3$seasonal)
#plot(decompose_ts_dollar_3$trend)
#plot(decompose_ts_dollar_3$random)
...
#plot(decompose_ts_dollar_4$seasonal)
#plot(decompose_ts_dollar_4$trend)
#plot(decompose_ts_dollar_4$random)
...
buildWordCloud(restaurant_reviews_rec$text, "Spectral",8,0, "test")
buildWordCloud(restaurant_reviews_norec$text, "Spectral",8,0, "test2")
