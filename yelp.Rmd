...
test1=gls(yelp_sp_diff~ts_users)
test2=lm(yelp_sp_diff~ts_users)
...
rec_dummy_rev_growth_q = 2:length(recession_dummy_reviews_q)
...
plot(seq(as.Date("2005/12/01"), by = "quarter",length.out = 44),restaurant_expenditures$real_exp,type="l")


#create var + granger causality for real exp and gdp
gdp__exp_combined = cbind(log_rev_quarter,gdp_growth_subset)
select=VARselect(gdp_combined,lag.max=12,type=c("const","trend","both","none"),season=NULL,exogen=NULL)
vm_gdp=VAR(gdp_combined,p=12)
plot(vm_gdp$y)
summary(vm_gdp)
